...
//             >
//               {isEnglish ? "Donate Now" : "ದೇಣಿಗೆ ನೀಡಿ"}
//             </button>
//           </form>
//         </div>
//       </section>
...
//     // Prevent further processing by returning immediately
//     return;

//     // Below code can be enabled once payment gateway is ready
//     /*
//     alert(
//       (isEnglish ? "Thank you for your donation, " : "ನಿಮ್ಮ ದಾನಕ್ಕೆ ಧನ್ಯವಾದಗಳು, ") +
//         `${formData.firstName} ${formData.lastName}`
...
const DonatePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirect user to payment gateway checkout page
      if (data.checkoutPageUrl) {
        window.location.href = data.checkoutPageUrl;
      } else {
        alert(isEnglish ? "Payment URL not available" : "ಪಾವತಿ URL ಲಭ್ಯವಿಲ್ಲ");
        setLoading(false);
      }
    } catch (error) {
      alert(isEnglish ? "Failed to submit donation" : "ದೇಣಿಗೆ ಸಲ್ಲಿಸುವಿಕೆ ವಿಫಲವಾಗಿದೆ");
      setLoading(false);
    }
  };
      ...
      <section>
        <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center items-center min-h-[80vh]">
          ...
          >
            ...
            <div>
              <label htmlFor="mobile" className="block mb-1 font-semibold text-gray-700">
                {isEnglish ? "Mobile Number" : "ಮೊಬೈಲ್ ಸಂಖ್ಯೆ"} *
              </label>
              <input
                id="mobile"
            ...
            <div>
              <label htmlFor="donationAmount" className="block mb-1 font-semibold text-gray-700">
                {isEnglish ? "Donation Amount (₹)" : "ದೇಣಿಗೆ ಮೊತ್ತ (₹)"} *
              </label>
              <input
                id="donationAmount"
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full font-bold py-3 rounded-md shadow transition-colors duration-300 ${
                loading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 text-white"
              }`}
            >
              {loading ? (isEnglish ? "Processing..." : "ಪ್ರಕ್ರಿಯೆ ಮುಗಿಯುತ್ತಿದೆ...") : (isEnglish ? "Donate Now" : "ದೇಣಿಗೆ ನೀಡಿ")}
            </button>
          </form>
        </div>
      </section>
