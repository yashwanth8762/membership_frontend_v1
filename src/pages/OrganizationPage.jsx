...
//   };
//   OrganizationContent.table.total_kn = OrganizationContent.table.kn.reduce((sum, item) => sum + item.members, 0);
//   OrganizationContent.table.total_en = OrganizationContent.table.en.reduce((sum, item) => sum + item.members, 0);
//   return (
//     <>
//       <section>
...
const OrganizationPage = () => {
  ...
  OrganizationContent.table.total_kn = OrganizationContent.table.kn.reduce(
    (sum, item) => sum + item.members,
    0
  );
  OrganizationContent.table.total_en = OrganizationContent.table.en.reduce(
    (sum, item) => sum + item.members,
    0
  );
    ...
    <>
      <section>
        <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center items-center min-h-[80vh]">
          ...
          <div className="w-full max-w-4xl mx-auto mb-10">
            <ol className="list-decimal list-inside space-y-4 text-lg text-gray-700">
              {OrganizationContent.first[isEnglish ? "en" : "kn"].map(
                (item, index) => (
                  <li key={index} className="mb-4 leading-relaxed">
                    {item}
                  </li>
              ...
              <table className="w-full bg-white rounded-lg shadow-lg border-collapse">
                ...
                <tbody>
                  ...
                  <tr className="bg-gray-100 font-bold">
                    <td className="px-6 py-4 text-lg text-gray-900 border-t border-gray-300"></td>
                    <td className="px-6 py-4 text-lg text-gray-900 border-t border-gray-300">
                  ...
                  {section.items.map((item, index) => {
                    ...
                    return (
                      <div key={index} className="mb-3 leading-relaxed">
                        {hasNewlines ? (
                          <div>
                            <div className="mb-2">{itemText.split("\n")[0]}</div>
                            <div className="ml-4 text-gray-600">
                              {itemText
                                .split("\n")
                                .slice(1)
                ...
                <tbody>
                  {CommitteeMembers.map((item, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 border-b border-gray-200 whitespace-pre-line">
                        {item.name[isEnglish ? "en" : "kn"]}
                      </td>
                      <td className="px-4 py-3 border-b border-gray-200">
                        {item.position[isEnglish ? "en" : "kn"]}
                      </td>
                      <td className="px-4 py-3 border-b border-gray-200">
                        {item.occupation[isEnglish ? "en" : "kn"]}
                      </td>
                      <td className="px-4 py-3 border-b border-gray-200">
                        {item.mobile}
                      </td>
          ...
          <div className="w-full max-w-6xl mx-auto mb-14">
            ...
            <div className="overflow-x-auto">
              <table className="w-full bg-white rounded-lg shadow-lg border-collapse">
                <thead>
                  <tr className="bg-blue-600 text-white">
                    {organizationTalukMembers.heading[
                      isEnglish ? "tableHeading_en" : "tableHeading_kn"
                    ].map((heading, idx) => (
                      <th
                        key={idx}
                        className="px-4 py-3 text-left font-semibold text-md border-b border-blue-700"
                      >
                        {heading}
                      </th>
                    ))}
                ...
                <tbody>{/* Data rows to be added here */}</tbody>
              </table>
            </div>
          </div>
          ...
          <div className="w-full max-w-6xl mx-auto mb-14">
            ...
            <div className="overflow-x-auto">
              <table className="w-full bg-white rounded-lg shadow-lg border-collapse">
                <thead>
                  <tr className="bg-blue-600 text-white">
                    {organizationDistrictMembers.heading[
                      isEnglish ? "tableHeading_en" : "tableHeading_kn"
                    ].map((heading, idx) => (
                      <th
                        key={idx}
