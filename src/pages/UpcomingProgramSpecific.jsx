...
const UpcomingProgramSpecific = () => {
  ...
  useEffect(() => {
    axios.get(`${API_BASE_URL}upcommingprograms/${id}`)
      .then((response) => {
        setProgram(response.data.data);
        setLoading(false);
      })
      .catch(() => {
        setError(isEnglish ? "Error fetching program details!" : "ಕಾರ್ಯಕ್ರಮದ ವಿವರಗಳನ್ನು ತರಲು ದೋಷವಾಗಿದೆ!");
        setLoading(false);
      });
  }, [id, isEnglish]);
  ...
  if (!program) return <div className="text-center py-12">{isEnglish ? "No program found." : "ಯಾವುದೇ ಕಾರ್ಯಕ್ರಮ ಕಂಡುಬಂದಿಲ್ಲ."}</div>;
    ...
    <section className="py-12 px-4 sm:px-6 lg:px-8 bg-white min-h-screen">
      <article className="max-w-3xl mx-auto">
        ...
        <header className="mb-8">
          <h1 className="text-4xl font-extrabold text-gray-900 mb-4 leading-tight">{displayTitle}</h1>
          {program.program_date && (
            <p className="text-gray-500 text-lg mb-2"><span className="font-semibold">{isEnglish ? 'Date:' : 'ದಿನಾಂಕ:'}</span> {new Date(program.program_date).toLocaleDateString()}</p>
          )}
          {displayLocation && (
            <p className="text-gray-500 text-lg mb-2"><span className="font-semibold">{isEnglish ? 'Location:' : 'ಸ್ಥಳ:'}</span> {displayLocation}</p>
          )}
          {program.program_time && (
            <p className="text-gray-500 text-lg mb-2"><span className="font-semibold">{isEnglish ? 'Time:' : 'ಸಮಯ:'}</span> {new Date(`1970-01-01T${program.program_time}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
          )}
        </header>
        {Array.isArray(program.media_file) && program.media_file.length > 0 && program.media_file.find(media => media.image_url && media.image_url.full && media.image_url.full.high_res) ? (
          (() => {
            ...
            return (
              <img src={mainImgSrc} alt={firstImage.name?.original || displayTitle} className="w-full h-80 object-cover object-center rounded-2xl mb-8 shadow-lg" />
            );
          })()
        ) : (
        ...
        <section className="prose prose-lg max-w-none text-gray-800 mb-8">
          <h2 className="text-2xl font-bold mb-4">{isEnglish ? 'About this Program' : 'ಈ ಕಾರ್ಯಕ್ರಮ ಬಗ್ಗೆ'}</h2>
          <p>{displayAbout}</p>
        </section>
            ...
            <div className="flex flex-wrap gap-3">
              {program.media_file.map((media, i) => {
                if (
                  ...
                  program.media_file.findIndex(m => m.image_url && m.image_url.thumbnail && m.image_url.thumbnail.high_res) === i
                ) {
                  return null;
                }
...
export default UpcomingProgramSpecific;
