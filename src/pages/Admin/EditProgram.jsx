...
export default function EditProgram() {
  ...
  useEffect(() => {
    const fetchProgram = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE_URL}upcommingprograms/${id}`);
        ...
        if (res.ok && data.data) {
          ...
          setKLocation(data.data.k_location || '');
          setProgramDate(data.data.program_date ? data.data.program_date.slice(0, 10) : '');
          setProgramTime(data.data.program_time || '');
          ...
          notifyError(data.message || 'Failed to fetch program');
        }
      } catch (err) {
        notifyError('Failed to fetch program');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}upcommingprograms/${id}`, {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        notifyError(data.message || 'Failed to update program');
      }
    } catch (error) {
      notifyError('Failed to update program');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <AdminLayout>
      <div style={{ minHeight: '100vh', background: 'linear-gradient(120deg, #f0f4f8 0%, #e0e7ef 100%)', padding: '40px 0' }}>
        <div style={{ maxWidth: 520, margin: '0 auto', background: '#fff', borderRadius: 18, boxShadow: '0 8px 32px 0 rgba(31, 38, 135, 0.10)', padding: '2.5rem 2rem', position: 'relative' }}>
          <button
            onClick={() => navigate('/dashboard/program/list')}
            style={{
              position: 'absolute',
              left: 24,
              top: 24,
          ...
          <h2 style={{ textAlign: 'center', fontWeight: 800, fontSize: 30, marginBottom: 32, color: '#1e293b', letterSpacing: 0.5 }}>Edit Program</h2>
          {loading ? (
            <div>Loading...</div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                disabled={submitting || (mediaFile && !mediaId)}
                style={{
                  width: '100%',
                  padding: '12px 0',
                  borderRadius: 8,
                  background: submitting ? '#a5b4fc' : 'linear-gradient(90deg, #6366f1 0%, #06b6d4 100%)',
                  color: '#fff',
                  fontWeight: 700,
                  fontSize: 18,
                  border: 'none',
                ...
                onMouseOver={e => { if (!submitting) e.target.style.background = '#4338ca'; }}
                onMouseOut={e => { if (!submitting) e.target.style.background = 'linear-gradient(90deg, #6366f1 0%, #06b6d4 100%)'; }}
              >
