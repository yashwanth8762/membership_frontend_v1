        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center mb-8">
            ...
            <h2 className="text-3xl font-bold text-gray-800 ml-6">
              {isEnglish ? 'Notifications/Programs' : 'ಅಧಿಸೂಚನೆಗಳು/ಕಾರ್ಯಕ್ರಮಗಳು'}
            </h2>
          </div>
              ...
              return (
                <div key={idx} className="bg-gray-50 rounded-2xl shadow-md hover:shadow-2xl transform hover:scale-105 transition-all duration-300 flex flex-col overflow-hidden h-[430px] p-6">
                  ...
                  {imageSrc ? (
                    ...
                    <div className="w-full h-48 bg-gray-200 flex items-center justify-center rounded-xl text-gray-400 text-lg font-semibold mb-4">
                  ...
                  <div className="flex flex-col items-center mb-4 text-gray-600 text-sm">
                    {program.program_date && (
                      <div className="mb-1">
                        <span className="font-semibold">{isEnglish ? 'Date:' : 'ದಿನಾಂಕ:'}</span>{" "}
                        {new Date(program.program_date).toLocaleDateString()}
                      </div>
                    )}
                    {displayLocation && (
                      <div className="mb-1">
                        <span className="font-semibold">{isEnglish ? 'Location:' : 'ಸ್ಥಳ:'}</span>{" "}
                        {displayLocation}
                      </div>
                    )}
                    {program.program_time && (
                      <div>
                        <span className="font-semibold">{isEnglish ? 'Time:' : 'ಸಮಯ:'}</span>{" "}
                        {new Date(`1970-01-01T${program.program_time}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    )}
                  </div>
